...
complicate separating those two factors in any analysis.
...
3. Total CSO Volumes and Events statewide are dropping even faster than in
   our region, so the percentage of the state's CSOs from here is climbing.  
...
We have no outliers with high leverage.  These models are actually not bad.
...
The relationship between slope based on log-transformed variables and percent 
change is simply the following:
...
years of our record.  Comparisons of those quanitites  better capture the 
type of long-term trend in CSO discharges that our reviewers wanted highlighted.
